...
for version in 1.13.0 1.14.0; do
  ...
  ucode -e '
    ...
    for (let rule in c.dns.rules) {
      ...
      // The final global source-aware fallback intentionally includes skipped devices.
      if (rule.server == "dnsmasq-server" && rule.type == "logical" &&
          index(sprintf("%J", rule), "192.0.2.0/24") >= 0)
